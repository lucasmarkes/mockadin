...
program
  .command('serve')
  .description('Start the mock API with hot reload')
  .option('-p, --port <number>', 'Server port', '3000')
  .option('-f, --from <dir>', 'Mocks directory', './mocks')
  .action(async (options) => {
    ...
    const mocksDir = path.resolve(process.cwd(), options.from);
    await loadMocks(mocksDir);
    startWatching(mocksDir);

    app.listen(options.port, () => {
      console.log(chalk.yellow(`🚀 Mock API running at http://localhost:${options.port}`));
    });
  });

// Command to create example mocks structure
program
  .command('init')
  .description('Create example mocks structure')
  .action(() => {
